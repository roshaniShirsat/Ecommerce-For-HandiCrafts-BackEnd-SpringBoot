...
public class CartItemController {
	...
	@DeleteMapping("/afterPayment")
	public ResponseEntity<ApiResponse>deleteCartAfterPayment(@RequestHeader("Authorization")String jwt) throws CartItemException, UserException{
		
		User user=userService.findUserProfileByJwt(jwt);
		System.out.println("User Cart Items to remove ---"+user.toString());
		cartItemService.removeCartItemAfterPayment(user.getId());
		
		ApiResponse res=new ApiResponse("Item Remove From Cart After Payement",true);
